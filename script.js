...
function renderMonitor() {
  ...
  ["WAITING", "INTERRUPTED", "SUBMITTED", "EXITED"].forEach(cat => {

    const btn = document.getElementById(`del${cat}Btn`);

    if (btn) {

      btn.disabled =
        attemptsByCategory(attempts, cat).length === 0;
...
/* Which attempts belong to a status card in Monitoring. */
function attemptsByCategory(attempts, category) {

  switch (category) {

    case "WAITING":
      return attempts.filter(a => a.status === "WAITING");
    ...
    case "EXITED":
      return attempts.filter(
        a =>
          a.status === "EXITED" &&
          a.reason !== "LEFT_SCREEN" &&
          a.reason !== "SIGNAL_LOST"
      );
...
/* Deletes every student record under one status card of the
   exam chosen in Monitoring. (Students who are still answering
   are never deleted: terminate them first.) */
async function deleteByStatus(category) {

  const examID = document.getElementById("monitorExam").value;
  ...
  if (!targets.length) {
    ...
    showToast("There are no records under this status.");
  ...
  if (
    !confirm(
      `Delete all ${targets.length} "${label}" record(s) ` +
      `of "${exam.title}"?\n\n` +
      "Their scores and answers will be permanently removed " +
      "from Monitoring and Results. This cannot be undone."
    )
  ) {
  ...
  try {
    ...
    db.attempts =
      db.attempts.filter(a => !gone.has(a.id));
    ...
    showToast(
      deleted === ids.length
        ...
        : `${deleted} of ${ids.length} records deleted.`
    );

  } catch (error) {

    console.error("EXAMGUARD bulk delete failed:", error);

    showToast("Could not delete the records. Try again.");
